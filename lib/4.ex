defmodule SquidBingo do
    ...
    result =
      Enum.reduce_while(
        ...
        fn {board_by_pos, board_by_num}, acc_boards ->
          ...
          case found do
            :yas ->
              {:halt, {:done, {new_board_by_pos, board_by_num}}}

            :nope ->
              {:cont, [{new_board_by_pos, board_by_num} | acc_boards]}
          end
        end
      )

    case result do
      {:done, board} -> {num, board}
  ...
  def solve() do
    ...
    {winning_num, winning_board} = mark_nums(numbers_parsed, boards_parsed)
    count_score(winning_num, winning_board)
  end
end
